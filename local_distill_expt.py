...
from utils import *
from multi_modules_expts import MultiNetwork, MultiNetworkFourierEmbedding, \
    MultiNetworkLinear, ShapeEmbedding
...
def create_shape_embedding(num_networks, embedding_dim):
    shape_embedding = ShapeEmbedding(num_networks, embedding_dim)
    num_input_channels = embedding_dim
    print("Embedding size ", shape_embedding.embed.weight.data.shape)
    return num_input_channels, shape_embedding
...
def preprocess_examples(batch_examples, domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, shape_embedding):
    num_networks = batch_examples.shape[0]
    batch_size = batch_examples.shape[1]
    embedding_idx = torch.unsqueeze(torch.unsqueeze(torch.arange(num_networks), -1).repeat(1, batch_size), -1).to(domain_mins).long()
    shape_embedding.to(domain_mins)
    # print("ShapeEmbedding shape ", embedding_idx.type(), embedding_idx.shape, batch_examples.shape)

    shape_embedding_input = shape_embedding(embedding_idx)
    # print("ShapeEmbedding shape ", shape_embedding_input.shape, embedding_idx.shape)
    if cfg['outputs'] == 'density':
        ...
        batch_inputs = torch.cat((batch_positions, shape_embedding_input), dim=2)
        ...
        batch_inputs = torch.cat((batch_positions, shape_embedding_input, batch_directions), dim=2)
...
def train_and_test_multi_network(multi_network, all_examples, domain_mins, domain_maxs, position_fourier_embedding, direction_fourier_embedding, shape_embedding, processing_saturated_nodes, cfg):
    ...
    params = list(multi_network.parameters())
    params.extend(list(shape_embedding.parameters()))
    ...
    # print("Size of input ", train_examples.shape, test_examples.shape)
    ...
    for iteration in range(1, cfg['iterations'] + 1):
        ...
        train_batch_inputs, train_batch_targets = preprocess_examples(train_examples[:, indices].to(domain_mins), domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, shape_embedding)
        # print("Size of preprocess_examples ", train_batch_inputs.shape, train_batch_targets.shape)
        ...
        # l2_reg = (shape_embedding.embed.weight ** 2).mean(dim=1).sum()/0.1
        l2_size_loss = torch.sum(torch.norm(shape_embedding.embed.weight.data, dim=1))
        code_reg_lambda = 1e-4
        reg_loss = (code_reg_lambda * min(1, iteration / 10000) * l2_size_loss)
        loss = loss.mean(dim=2).mean(dim=1).sum() + reg_loss
                            
        # print("Embedding loss ", shape_embedding.embed.weight.shape, reg_loss, loss)
        ...
        if iteration % cfg['test_every'] == 0 or iteration == cfg['iterations']:
            errors_per_point, errors_per_network, errors_per_network_color, errors_per_network_density, saturation =\
                test_multi_network(multi_network, test_examples, domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, shape_embedding)
            ...
            for network_index in range(num_networks):
                ...
                if 'performance_measurement' in cfg:
                    ...
                    print("Time taken to process ", network_index, (time.time() - start_time))
                for metric in list_metrics():
...
def test_multi_network(multi_network, test_examples, domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, shape_embedding):
    ...
    with torch.no_grad():
        ...
        while start < num_test_examples:
            ...
            test_batch_inputs, test_batch_targets = preprocess_examples(test_examples[:, start:end].to(domain_mins), domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, shape_embedding)
...
def train_and_test_nodes(node_batch, pretrained_nerf, processing_saturated_nodes, cfg, dev):
    ...
    embedding_dim = 512
    shape_num_input_channels, shape_embedding = \
        create_shape_embedding(num_networks, embedding_dim)
    ...
    multi_network =  create_multi_network(num_networks, position_num_input_channels+shape_num_input_channels, 
        direction_num_input_channels, 4, 'bmm', cfg).to(dev)
    ...
    #     # print("Input generator ", network_index, num_networks, time.time() - start_time)
    ...
    #         del raw_output
    #         # print("pretrained inference ", start, end, time.time() - start_time)
    #         start = end
    # if cfg['outputs'] == 'density':
    ...
    # torch.save(all_examples, './logs/distill/Synthetic_NeRF_Lego/all_examples.pt')
    all_examples = torch.load('./logs/distill/Synthetic_NeRF_Lego/all_examples.pt')
    print("Done saving the generated points ")
    # torch.save(all_examples, './logs/distill/Synthetic_NeRF_Lego/all_examples.pt')
    ...
    test_points, errors_per_point, best_errors_per_network, best_errors_per_network_color, best_errors_per_network_density, saturation =\
        train_and_test_multi_network(multi_network, all_examples, domain_mins, domain_maxs, position_fourier_embedding, direction_fourier_embedding, shape_embedding, processing_saturated_nodes, cfg)
    return multi_network, test_points, errors_per_point, best_errors_per_network, best_errors_per_network_color, best_errors_per_network_density, saturation, shape_embedding
...
def train(cfg, log_path):
    ...
    while cp['phase'] == 'discovery' and (cp['nodes_to_process'] or cp['saturated_nodes_to_process']) and not killer.kill_now and not restarting_job:
        ...
        num_networks = len(node_batch)
        multi_network, test_points, errors_per_point, best_errors_per_network, best_errors_per_network_color, best_errors_per_network_density, saturation, shape_embedding =\
        ...
        cp['shape_embedding'] = shape_embedding.embed.weight.data
