...
from local_distill_expt import create_multi_network_fourier_embedding, has_flag, create_multi_network, \
    create_shape_embedding
from multi_modules_expts import build_multi_network_from_single_networks, \
    extract_linears, query_multi_network, update_shared_weights
    ...
    def query_network():
        if network_query_fn is not None:
            ...
            return query_multi_network(multi_network, domain_mins, domain_maxs, pts, viewdirs,
                shape_embedding, position_fourier_embedding, direction_fourier_embedding, occupancy_grid, debug_network_color_map, use_network_jittering, random_directions, cfg)
...
def train(cfg, log_path, render_cfg_path):
    ...
    # print("finetuning_distilled ", finetuning_distilled)
    ...
    if cfg['model_type'] == 'single_network':
        ...
        # num_networks = 1440
        embedding_dim = 512
        shape_num_input_channels, shape_embedding = create_shape_embedding(num_networks, embedding_dim)
        ...
        if load_from_distilled:
            shape_embedding.embed.weight.data = cp['shape_embedding']
        ...
        if not load_from_distilled:
            ...
            # num_networks = 1440
            print("The number of networks ", num_networks, res)
            model = multi_network = create_multi_network(num_networks, position_num_input_channels+shape_num_input_channels, 
                direction_num_input_channels, 4, 'multimatmul_differentiable', cfg).to(device)
            ...
            cp = torch.load("logs/paper/finetune/Synthetic_NeRF_Lego/checkpoint_1000000.pth")
            print("List checkpoint ", cp.keys())
            root_nodes = cp['root_nodes']
            print("Root Nodes ", len(root_nodes))
            update_shared_weights(root_nodes.copy()[0].network, multi_network)

        # From distillation
        else:
            root_nodes = cp['root_nodes']

            print("List of networks ", len(root_nodes), dir(root_nodes[0]))
    ...
    params = list(shape_embedding.parameters())
    # params.extend(list(model.parameters()))

    optimizer_type = cfg['optimizer_type'] if 'optimizer_type' in cfg else 'adam'
    if optimizer_type == 'adam':
        optimizer = torch.optim.Adam(params=params, lr=cfg['initial_learning_rate'], betas=(0.9, 0.999))
    elif optimizer_type == 'sgd':
        optimizer = torch.optim.SGD(params=params, lr=cfg['initial_learning_rate']) # no momentum

    start = 0
    print("Start load_from_distilled ", load_from_distilled, load_from_checkpoint)
    if load_from_checkpoint and not load_from_distilled:
        optimizer.load_state_dict(cp1['optimizer_state_dict'])
        print("The model state dict ", cp1['model_state_dict'].keys())
        print("Actual model keys ", model.state_dict().keys())
        model.load_state_dict(cp1['model_state_dict'])
    ...
    if cfg['render_only']:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path(render_poses, intrinsics, cfg['chunk_size'], 
    ...
    start = start + 1
    print("Start and end ", start, N_iters)
    ...
    for i in trange(start, N_iters):
        ...
        if do_constant_pretraining: # assuming local coordinates
            ...
            if 'l2_regularization_lambda' in cfg:
                ...
                # print("L2 regularization param ", l2_reg_term)
            ...
            reg_loss = (code_reg_lambda * l2_size_loss)
            # print("Embed size loss ", l2_size_loss, loss, reg_loss)
            # loss = loss + reg_loss
        ...
        if i % cfg['render_testset_interval'] == 0 and i > 0:
            ...
            model.eval()
            with torch.no_grad():
                rgbs, disps = render_path(torch.Tensor(poses[i_test]).to(device), intrinsics, cfg['chunk_size'], render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir)
                del rgbs
                del disps
                gc.collect()
        ...
        if i % cfg['checkpoint_interval'] == 0:
            ...
            cp = {
                ...
                'shape_embedding': shape_embedding.embed.weight.data,
                'root_nodes': root_nodes
            }
