...
class ShapeEmbedding(nn.Module):
    def __init__(self, num_networks, embedding_dim):
        super(ShapeEmbedding, self).__init__()
        
        self.num_networks = num_networks
        self.embedding_dim = embedding_dim
        self.embed = nn.Embedding(num_networks, embedding_dim)
        nn.init.normal_(self.embed.weight.data, 0.0, 1.0 / math.sqrt(embedding_dim))
    ...
    def forward(self, input):
        ...
        if has_network_dim:
            out = out.view(num_networks, -1, self.embedding_dim)
        return out
...
class MultiNetwork(nn.Module):
    def __init__(self, num_networks, num_position_channels, num_direction_channels, num_output_channels, hidden_layer_size, num_hidden_layers, refeed_position_index=None, late_feed_direction=False,
        ...
        use_hard_parameter_sharing = True
        self.use_hard_parameter_sharing_for_color = True
        ...
        if self.late_feed_direction:
            self.pts_linears = [linear_layer(self.num_position_channels, self.hidden_layer_size, self.nonlinearity, use_hard_parameter_sharing)]
            nonlinearity_params = nonlinearity_params.copy().update({'is_first': False})
            for i in range(self.num_hidden_layers - 1):
                if i == self.refeed_position_index:
                    new_layer = linear_layer(self.hidden_layer_size + self.num_position_channels, self.hidden_layer_size, self.nonlinearity, use_hard_parameter_sharing)
                else:
                    new_layer = linear_layer(self.hidden_layer_size, self.hidden_layer_size, self.nonlinearity, use_hard_parameter_sharing)
            ...
            self.num_hidden_dir_layers = 4
            for i in range(self.num_hidden_dir_layers - 1):
                 new_layer = linear_layer(self.direction_layer_size, self.direction_layer_size, self.nonlinearity, use_hard_parameter_sharing)
                 self.direction_layer.append(new_layer)
            self.direction_layer = nn.ModuleList(self.direction_layer)
            ...
            self.feature_linear = linear_layer(self.hidden_layer_size, feature_output_size, 'linear', use_hard_parameter_sharing)
            ...
            if not self.use_view_independent_color:
                self.alpha_linear = linear_layer(self.hidden_layer_size, 1, 'linear' if self.alpha_rgb_initalization == 'updated_yenchenlin' else 'relu', use_hard_parameter_sharing)
    ...
    def forward(self, x, batch_size_per_network=None, random_directions=None):
        if self.late_feed_direction:
            ...
            del feature
            del directions
            for i, l in enumerate(self.direction_layer):
                h = self.direction_layer[i](h, batch_size_per_network)
                PerfMonitor.add('direction_linear '+ str(i), ['network query', 'matmul'])
                h = self.activation(h)
            if self.view_dependent_dropout_probability > 0:
...
def query_multi_network(multi_network, domain_mins, domain_maxs, points, directions,
    ...
    # print("Dimensions of inputs ", embedded_points.shape, embedded_dirs.shape, 
        # filtered_point_indices.shape)
    if shape_embedding is not None:
        embedded_shape = shape_embedding(filtered_point_indices.unsqueeze(0)).squeeze(0)
    else:
        embedded_shape = filtered_point_indices
    del filtered_point_indices
    embedded_points = torch.cat((embedded_points, embedded_shape), dim=-1)
    # print("Embedding shape ", embedded_shape.shape, embedded_points.shape)
...
def update_shared_weights(single_network, multi_network):
    multi_linears, multi_shared_linears = extract_linears(multi_network)
    linears_per_network = extract_linears(single_network)
    num_linear_layers = len(multi_linears)
    num_linear_layers_shared = len(multi_shared_linears)
    with torch.no_grad():
        for layer_index in range(num_linear_layers_shared):
            new_weight = linears_per_network[1][layer_index].weight.data
            new_bias = linears_per_network[1][layer_index].bias.data
            print("Dimension ", new_weight.shape,
                multi_shared_linears[layer_index].weight.data.shape)
            multi_shared_linears[layer_index].weight.data = new_weight
            multi_shared_linears[layer_index].bias.data = new_bias
